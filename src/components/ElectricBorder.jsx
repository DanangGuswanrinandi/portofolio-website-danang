...
const ElectricBorder = ({
  children,
  color = '#8A00FF',     // default neon purple
  speed = 1,
  chaos = 1,
  thickness = 3,
  className = '',
  ...
  const strokeStyle = {
    ...
    borderStyle: 'solid',
    borderColor: color,
    boxShadow: `0 0 ${thickness * 2}px ${hexToRgba(color, 0.9)}, 0 0 ${thickness * 5}px ${hexToRgba(color, 0.6)}`,
  };

  // Two glow layers for neon effect
  const glow1Style = {
    ...inheritRadius,
    borderWidth: thickness,
    borderStyle: 'solid',
    borderColor: hexToRgba(color, 0.5),
    filter: `blur(${thickness * 1.5}px)`,
    opacity: 0.7,
    boxShadow: `0 0 ${thickness * 3}px ${hexToRgba(color, 0.5)}`,
  };
  ...
  const glow2Style = {
    ...
    opacity: 0.5,
    boxShadow: `0 0 ${thickness * 6}px ${hexToRgba(color, 0.4)}`,
  };

  // Background glow behind stroke
  const bgGlowStyle = {
    ...inheritRadius,
    transform: 'scale(1.1)',
    filter: 'blur(20px)',
    opacity: 0.4,
    background: `linear-gradient(45deg, ${hexToRgba(color, 0.8)}, ${hexToRgba(color, 0.3)}, transparent)`,
    zIndex: -1
  };
  ...
  return (
    <div ref={rootRef} className={`relative isolate ${className ?? ''}`} style={style}>
      ...
      {/* Neon wave / border overlay */}
      <div className="absolute inset-0 pointer-events-none z-20" style={inheritRadius}>
      ...
      {/* Konten di bawah border overlay */}
